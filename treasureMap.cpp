...
void treasureMap::setLOB(PNG & im, pair<int,int> loc, int d) {
    ...
    Pixel->r = r | (v >> 4);
    Pixel->g = g | (v >> 2 & 0b00000011);
    Pixel->b = b | (v & 0b00000011);
}

PNG treasureMap::renderMap() {
    PNG treasure = base;

    vector<vector<bool>> visited(treasure.height(), vector<bool>(treasure.width(), false));

    vector<vector<int>> path(treasure.height(), vector<int>(treasure.width(), -1));
